...
async function main() {
  ...
  const c = await hre.ethers.deployContract('IBCBridge', [getUcHandlerAddress(hre.network.name)]);
  ...
  console.log(
    `Contract IBCBridge deployed to ${c.target} on network ${hre.network.name}`
  );
}
